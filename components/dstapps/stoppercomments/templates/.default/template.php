...
for($i = 0; $i < count($com); $i++) {
  ...
  $date = new DateTime('@1654688422005');
  $ts = $date->format('m.d.y');

// Можно через setTimestamp


  // $com[$i]['date'] = date("m.d.y", $ts);
  $com[$i]['date'] = $ts;
}
print_r($tasks);
echo "<pre>";
...
<body class="task-stopper-slider-body">
    ...
    <div class="task-stopper-slider-workarea">
        ...
        <?
          ...
          ?>
            ...
            <input id="saveComment" type="submit" class="ui-btn ui-btn-success"  name="submit"  value="Поставить стоппер" title="Сохранить">
  ...
  <!-- Comments List -->
    ...
    <!-- Comment -->
    <!-- ngRepeat: comment in cmntCtrl.comments | orderBy: '-date' -->
    ...
    <div class="comment">
      ...
      <div class="comment-box">
        ...
        <div class="comment-footer">
          ...
          <? } else { ?>
            <p><b>Комментариев к стопперу нет</b></p>
          ...
          <? } else { ?> 
            <!-- сценарий, если стопер уже есть, рисуем комментарии к поставленному стопперу  -->
            ...
            <? foreach ($com as $value) { ?>
              <section>
         <blockquote>
            <p><? echo $value['comment_text'];?></p>
            <footer>— <cite><? echo $value['user'];?></cite></footer>
         </blockquote>
          </section>
            <? } ?>
  ...
  <script>
    ...
    saveCommentButton.onclick = async function () {
    ...
    if(commentsTextValue != '' && commentsTextValue != null) {
      let task = await Application.sendAjax('dstapps:ownkanban', 'testinfo', 'class', { className: 'OwnKanbanTasks', methodName: 'setTasksStopperComments', params: { taskId: taskId, userId: userId, commentsText: commentsTextValue, date: date } });
      console.log(task,'ы');
      let task2 = await Application.sendAjax('dstapps:ownkanban', 'testinfo', 'class', { className: 'OwnKanbanTasks', methodName: 'setTasksStopper', params: { taskId: taskId, } });

      BX.SidePanel.Instance.postMessage(window, "onMessage", { taskId: taskId });
